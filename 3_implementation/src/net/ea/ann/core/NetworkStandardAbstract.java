...
public abstract class NetworkStandardAbstract extends NetworkAbstract implements NetworkStandard {
	...
	public List<LayerStandard> getAllLayers() {
		...
		List<LayerStandard> main = Util.newList(0);
		if (inputLayer != null) main.add(inputLayer);
		main.addAll(hiddenLayers);
		if (outputLayer != null) main.add(outputLayer);
		if (memoryLayer != null) main.add(memoryLayer);
		
		for (LayerStandard layer : main) {
	...
	protected static NeuronValue[] evaluate(List<LayerStandard> bone, NeuronValue[] input) {
		...
		for (int j = 0; j < layer0.size(); j++) {
			NeuronStandard neuron = layer0.get(j);
	...
	private NeuronValue evaluate(NeuronStandard neuron, boolean refresh) {
		...
		neuron.setInput(in);
		NeuronValue out = neuron.getActivateRef().evaluate(in);
		neuron.setOutput(out);
		return out;
	}
	 ...
	 */
	protected abstract NeuronValue calcOutputError(NeuronStandard outputNeuron, NeuronValue realOutput, LayerStandard outputLayer);
	...
	/**
	 * Convert this network to text.
	 * @return text representing this network.
	 */
...
//					NeuronStandard neuron = nextLayer.get(j);
...
//			NeuronStandard centerNeuron = centerLayer.get(j);
