...
public class NetworkStandardImpl extends NetworkStandardAbstract {
	 ...
	 */
	public Function getActivationRef() {
		return activateRef;
	}
...
//	/**
//	 * Learning neural network by back propagate algorithm.
//	 * @param epoch learning sample.
//	 * @param learningRate learning rate.
//	 * @param terminatedThreshold terminated threshold.
//	 * @param maxIteration maximum iteration.
//	 * @return learned error.
//	 */
//	public synchronized NeuronValue[] bpLearnEpoch(Iterable<Record> epoch, double learningRate, double terminatedThreshold, int maxIteration) {
//		try {
//			if (isDoStarted()) return null;
//		} catch (Throwable e) {Util.trace(e);}
//		
...
//		doStarted = true;
//		while (doStarted && (maxIteration <= 0 || iteration < maxIteration)) {
//			List<NeuronValue[]> outputEpoch = Util.newList(0);
//			NeuronValue[] input = null;
//			for (Record record : epoch) {
//				if (record.input != null) input = record.input;
//				outputEpoch.add(record.output);
//			}
//
//			if (input != null) {
//				//Evaluating layers.
...
//					evaluate(inputRecord, true);
//				} catch (Throwable e) {Util.trace(e);}
//			}
//			
//			//Updating weights and biases of backbone.
//			error = bpUpdateWeightsBiases(backbone, outputEpoch, learningRate);
//			
//			List<List<LayerStandard>> ribinbones = getRibinbones();
//			for (List<LayerStandard> ribinbone : ribinbones) {
//				if (ribinbone.size() < 2) continue;
...
//	}
	 ...
	 * @return learned error.
	 */
	public NeuronValue[] bpLearn(List<LayerStandard> bone, double learningRate, double terminatedThreshold, int maxIteration) {
		return bpLearn(bone, (Iterable<NeuronValue[]>)null, learningRate, terminatedThreshold, maxIteration);
	}
	 ...
	 * @return learned error.
	 */
	public NeuronValue[] bpLearn(List<LayerStandard> bone, NeuronValue[] input, double learningRate, double terminatedThreshold, int maxIteration) {
		return bpLearn(bone, input, (NeuronValue[]) null, learningRate, terminatedThreshold, maxIteration);
	}
	...
	public NeuronValue[] bpLearn(List<LayerStandard> bone, NeuronValue[] input, NeuronValue[] output, double learningRate, double terminatedThreshold, int maxIteration) {
		if (output == null)
			return bpLearn(bone, input, (Iterable<NeuronValue[]>)null, learningRate, terminatedThreshold, maxIteration);
		
		List<NeuronValue[]> outputEpoch = Util.newList(1);
		outputEpoch.add(output);
		return bpLearn(bone, input, outputEpoch, learningRate, terminatedThreshold, maxIteration);
	}
	 ...
	 * Learning bone by back propagate algorithm.
	 * @param bone list of layers including input layer.
	 * @param outputEpoch output epoch. It can be null;
	 * @param learningRate learning rate.
	 * @param terminatedThreshold terminated threshold.
	 * @param maxIteration maximum iteration.
	 * @return learned error.
	 */
	public NeuronValue[] bpLearn(List<LayerStandard> bone, Iterable<NeuronValue[]> outputEpoch, double learningRate, double terminatedThreshold, int maxIteration) {
		...
		while (maxIteration <= 0 || iteration < maxIteration) {
			//Do not evaluate layers here.

			//Updating weights and biases of backbone.
			error = bpUpdateWeightsBiases(bone, outputEpoch, learningRate);
	 ...
	 * @return learned error.
	 */
	public NeuronValue[] bpLearn(List<LayerStandard> bone, NeuronValue[] input, Iterable<NeuronValue[]> outputEpoch, double learningRate, double terminatedThreshold, int maxIteration) {
		if (bone == null || bone.size() < 2) return null;
	 ...
	 */
	private NeuronValue[] bpUpdateWeightsBiases(List<LayerStandard> bone, NeuronValue[] output, double learningRate) {
		if (output == null) bpUpdateWeightsBiases(bone, (Iterable<NeuronValue[]>)null, learningRate);

		List<NeuronValue[]> outputEpoch = Util.newList(1);
		outputEpoch.add(output);
		return bpUpdateWeightsBiases(bone, outputEpoch, learningRate);
	}
	 ...
	 * Updating weights and biases.
	 * @param bone list of layers including input layer.
	 * @param outputEpoch output epoch of output layer. 
	 * @param learningRate learning rate.
	 * @return errors of output errors. Return null if errors occur.
	 */
	protected NeuronValue[] bpUpdateWeightsBiases(List<LayerStandard> bone, Iterable<NeuronValue[]> outputEpoch, double learningRate) {
		...
		for (int i = bone.size() - 1; i >= 1; i--) {
			...
			for (int j = 0; j < layer.size(); j++) {
				...
				if (i == bone.size() - 1) {
					if (outputEpoch == null)
						error[j] = calcOutputError(neuron, null, layer); //There are many cases that no need of output.
					else {
						int n = 0;
						error[j] = layer.newNeuronValue().zero();
						for (NeuronValue[] output : outputEpoch) {
							if (output != null)
								output = NeuronValue.adjustArray(output, bone.get(bone.size()-1).size(), bone.get(bone.size()-1));
							NeuronValue e = calcOutputError(neuron, output != null? output[j] : null, layer);
							if (e == null) continue;
							
							error[j] = error[j].add(e);
							n++;
						}
						if (n != 0) error[j] = error[j].multiply(1.0/(double)n);
					}
				}
				else {
					LayerStandard nextLayer = bone.get(i + 1);
					NeuronValue rsum = layer.newNeuronValue().zero();
	...
	@Override
	protected NeuronValue calcOutputError(NeuronStandard outputNeuron, NeuronValue realOutput, LayerStandard outputLayer) {
		NeuronValue out = outputNeuron.getOutput();
		NeuronValue derivative = outputNeuron.getActivateRef().derivative(out); //This is the trick to squash the derivative at output as at input.
		return realOutput.subtract(out).multiplyDerivative(derivative);
	}
