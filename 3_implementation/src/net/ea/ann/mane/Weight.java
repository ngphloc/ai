...
public class Weight implements Cloneable, Serializable {
	 ...
	 */
	static MatrixStack[] createKernel(double kernelValue, Size size, Matrix kernelHint) {
		if (size.width < 1 || size.height < 1 || kernelHint == null) return null;
		int depth = 1, time = 1;
		if (size.depth < 1)
			time = depth = 1;
		else if (size.time < 1) {
		...
		NeuronValue value = kernelHint.get(0, 0).valueOf(kernelValue);
		for (int t = 0; t < time; t++) {
			Matrix matrix = kernelHint.create(new Size(size.width, size.height, depth, 1)); 
...
/**
 * This class represents parametric weight kernel.
 * 
 * @author Loc Nguyen
 * @version 1.0
 *
